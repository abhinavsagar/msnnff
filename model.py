...
def create_model(existing='', is_twohundred=False, is_halffeatures=True):
    ...
    if len(existing) == 0:
        ...
        # Encoder Layers
        if is_twohundred:
            base_model = applications.ResNet101(input_shape=(None, None, 3), include_top=False)
        else:
            base_model = applications.ResNet152(input_shape=(None, None, 3), include_top=False)
